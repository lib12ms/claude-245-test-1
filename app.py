...
def _nlk_scrape_orig_title(control_no: str) -> str | None:
...
def _nlk_holdings_control_no_xml(isbn13: str, key: str) -> str | None:
...
def fetch_nlk_orig_title_by_isbn(isbn13: str) -> str | None:
    ...
    def _scan_orig_only(s: str) -> None:
        if not s:
            return
        ot = extract_orig_title_from_nlk_text(s)
        if ot:
            orig_candidates.append(ot)
        par = extract_equals_parallel_title(s)
        if par:
            orig_candidates.append(par)

    def _try_holdings_json() -> None:
        params = {
            "key": key,
            "apiType": "json",
            "searchType": "isbn",
        ...
        try:
            resp = requests.get(NLK_SEARCH_API_URL, params=params, timeout=12)
            resp.raise_for_status()
            data = resp.json()
    ...
    def _try_seoji() -> None:
        ...
        try:
            resp = requests.get(NLK_SEOJI_API_URL, params=params, timeout=12)
            resp.raise_for_status()
            data = resp.json()
        except (requests.RequestException, ValueError):
            return
    ...
    control_no = _nlk_holdings_control_no_xml(isbn13, key)
    if control_no:
        scraped = _nlk_scrape_orig_title(control_no)
        if scraped:
            return scraped
...
def _isbn_lookup():
    ...
    # 246: 알라딘에 원서명이 없을 때만 국중 Open API에서 원표제 보강
    if not orig_title:
        orig_title = fetch_nlk_orig_title_by_isbn(isbn13)

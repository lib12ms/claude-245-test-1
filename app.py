...
def _isbn_lookup():
    ...
    print(f"[DEBUG] isbn={isbn13} author_str={author_str!r} has_translator={has_translator}", flush=True)
    ...
    if needs_orig_lookup:
        ...
        print(f"[DEBUG2] orig_author_en after collect={orig_author_en!r}", flush=True)
        ...
        if author_info:
            ...
            if thin:
                ...
                print(f"[DEBUG3] thin={thin} names={names!r} threshold={max(2, n_comm+2)}", flush=True)
                if names and len(names) >= max(2, n_comm + 2):
                    orig_author_en = ", ".join(names[: min(len(names), n_auth + 4)])
                    print(f"[DEBUG3] orig_author_en overwritten={orig_author_en!r}", flush=True)

...
def _nlk_seoji_rows(
    ...
    def _fetch(params: dict, via: str) -> None:
        ...
        if rows and meta.get("result") == "SUCCESS":
            return
        try:
            resp = requests.get(NLK_SEOJI_API_URL, params=params, timeout=max(3, timeout))
            resp.raise_for_status()
            data = resp.json()
        except (requests.RequestException, ValueError) as e:
            meta.setdefault("errors", []).append(f"{via}: {e}")
            return
    ...
    if not rows and hint and len(hint) >= 2:
        _fetch({
            "cert_key": key,
            "result_style": "json",
            "page_no": 1,
...
def _nlk_holdings_json_orig_candidates(
    ...
    try:
        resp = requests.get(NLK_SEARCH_API_URL, params=params, timeout=max(3, timeout))
        resp.raise_for_status()
        data = resp.json()
    except (requests.RequestException, ValueError) as e:
        meta["error"] = str(e)
...
def fetch_nlk_orig_title_by_isbn(
    ...
    budget = float(os.environ.get("NLK_FETCH_BUDGET_SEC", "14"))
...
def _isbn_lookup():
    ...
    nlk_status: dict = {}
    if has_translator:
        # 알라딘 수집과 국중 원표제(246)를 병렬 — Render 공개 URL ~30초 제한
        nlk_fut = None
        with ThreadPoolExecutor(max_workers=2) as pool:
            fut_info = pool.submit(
                collect_orig_info, item, item_id, isbn13, title, authors
            )
            if nlk_configured:
                nlk_fut = pool.submit(
                    fetch_nlk_orig_title_by_isbn,
                    isbn13,
                    title,
                    nlk_status,
                )
            orig_info = fut_info.result()
            nlk_title = nlk_fut.result() if nlk_fut else None
    ...
    payload = {
        ...
        "nlk_status": nlk_status if has_translator and nlk_configured else None,
    }
    try:

...
def enrich_kanji_map_from_author_overview_pages(
    ...
    for nm in names:
        ...
        if need_e and en_inner and _norm_author_display(outer_en) == _norm_author_display(key):
...
def try_orig_names_500_from_profile_english(
    ...
    """저자 프로필 등으로 채운 english_name 이 author_info 전원에 있으면 500용 영문 리스트."""
...
def _isbn_lookup():
    ...
    if orig_names_500_from_intro is not None:
        ...
        orig_names_500 = orig_names_500_from_profiles
        for ai, en in zip(author_info, orig_names_500_from_profiles):
            ai["_500_script"] = "roman"
            ai["_roman_for_900"] = en
    else:
        orig_names_500 = []
    ...
    western_700_english = (
        ...
        and all((ai.get("english_name") or "").strip() for ai in author_info)
    )

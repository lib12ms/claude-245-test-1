...
[생성 필드]
  ...
  710 0_  기관명 부출기입
  900 10  원저자 한글명 부출 (500 원문이 한자만·히라가나 없을 때만 음역; 히라가나만 또는 한자+히라가나 혼용은 900 생략; 500이 로마자인 서양 원저는 build_900 규칙)
"""
...
def enrich_kanji_map_from_author_overview_pages(
    ...
    for nm in names:
        ...
        for attempt in range(2):
            try:
                resp = requests.get(url, headers=ALADIN_HEADERS, timeout=14)
                resp.raise_for_status()
        ...
        if script and _norm_author_display(ko) == _norm_author_display(key):
            km[key] = script
        time.sleep(0.12)
...
def build_900(
    ...
    """
    - 일본 맥락: 한자·가나 원표기(kanji_name)가 있으면 일본어 음 한글이 245와 다를 때만 그걸 900에 쓰고, 같거나 없으면 245와 같은 한글 순서(성·이름 도치 없음).
    - 번역서이면서 동아시아 원표기가 없고 영문 원저자명이 있으면(서양): 한글 성·이름 도치(예: 프랑수아즈 사강 → 사강, 프랑수아즈).
    ...
    if translation_book or japanese_family_book:
    ...
    if field_500:
        for idx, ai in enumerate(author_info):
            if ai.get("_500_script") == "cjk":
                kn = (ai.get("kanji_name") or "").strip()
                has_kanji = bool(re.search(r"[\u4e00-\u9fff\u3400-\u4dbf]", kn))
                has_hiragana = bool(re.search(r"[\u3040-\u309f]", kn))
                if not has_kanji or has_hiragana:
                    continue
                gloss = _jp_name_reading_hangul(kn)
                if gloss:
            ...
            f900 = build_900(
                ...
                japanese_family_book=japanese_family_900,
            )
            if f900:

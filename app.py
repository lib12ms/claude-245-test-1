...
def fetch_nlk_responsibility_statement(isbn13: str) -> str | None:
    """
    국립중앙도서관 Open API(소장자료·ISBN 서지)로 책임표시 조회.
    환경변수 NLK_API_KEY(인증키) 필요 — 국중 회원가입 후 발급.
    """
    key = NLK_API_KEY
    if not key:
        return None
    ...
    def _try_seoji() -> None:
        ...
        try:
            ...
            return
        for row in data.get("docs") or data.get("DOC") or []:
            if not isinstance(row, dict):
                continue
            for fld in ("AUTHOR", "author", "TITLE", "title"):
                val = row.get(fld)
                if not val:
                    continue
                hit = extract_responsibility_from_nlk_bib_text(str(val))
                if hit:
                    candidates.append(hit)
    ...
    if not candidates:
        return None
    # 가장 많은 역할 구분(;)을 가진 문자열을 우선(합저·역자 구분이 잘 된 경우)
    return max(candidates, key=lambda s: (s.count(";"), len(s)))
...
def _isbn_lookup():
    ...
    # MARC 필드 생성
    nlk_responsibility = fetch_nlk_responsibility_statement(isbn13)
    field_245 = build_245(title, subtitle, authors, responsibility=nlk_responsibility)

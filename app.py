...
def _nlk_seoji_rows(isbn13: str, key: str, timeout: float, title_hint: str = "") -> list[dict]:
    ...
    def _fetch(params: dict) -> None:
        ...
        try:
            ...
            return
        if (data.get("RESULT") or "").upper() == "ERROR":
            return
...
def _nlk_holdings_json_orig_candidates(isbn13: str, key: str, timeout: float) -> list[str]:
...
def fetch_nlk_orig_title_by_isbn(
    ...
    tmo = lambda: min(8, max(3, _left()))
    orig_candidates: list[str] = []

    if _left() > 0:
        with ThreadPoolExecutor(max_workers=2) as pool:
            fut_seoji = pool.submit(
                _nlk_seoji_rows, isbn13, key, tmo(), title_hint
            )
            fut_hold = pool.submit(
                _nlk_holdings_json_orig_candidates, isbn13, key, tmo()
            )
            try:
                rows = fut_seoji.result(timeout=max(1, _left()))
                orig_candidates.extend(_nlk_orig_candidates_from_rows(rows))
            except Exception:
                pass
            try:
                orig_candidates.extend(fut_hold.result(timeout=max(1, _left())))
            except Exception:
                pass
    ...
    if control_no and _left() > 0:
        return _nlk_scrape_orig_title(control_no, timeout=min(8, max(3, _left())))
...
def _isbn_lookup():
    ...
    if has_translator:
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            ...
            if nlk_configured:
                nlk_fut = pool.submit(
                    fetch_nlk_orig_title_by_isbn, isbn13, title
                )

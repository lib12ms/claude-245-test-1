...
def fetch_gemini_orig_info(title: str, author_name: str, isbn13: str) -> dict:
    ...
    if not _GEMINI_AVAILABLE:
        print(f"[Gemini] google-generativeai 패키지 없음 — 스킵", flush=True)
        return result
    if not GEMINI_API_KEY:
        print(f"[Gemini] GEMINI_API_KEY 미설정 — 스킵", flush=True)
        return result

    print(f"[Gemini] 호출: title={title!r}, author={author_name!r}, isbn={isbn13}", flush=True)
...
{{"orig_title": "원서명 또는 null", "orig_author_en": "원저자 영문명 또는 null"}}"""
        ...
        print(f"[Gemini] 응답 원문: {text!r}", flush=True)
        ...
        print(f"[Gemini] 파싱 결과: {data}", flush=True)
        ...
        print(f"[Gemini] 최종 반환: {result}", flush=True)

    except Exception as e:
        print(f"[Gemini] 예외 발생: {type(e).__name__}: {e}", flush=True)

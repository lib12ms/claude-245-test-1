...
def collect_orig_info(item: dict, item_id: str, isbn13: str, title: str, authors: list[dict]) -> dict:
    ...
    print(f"[DEBUG] pre-NLK: orig_title={orig_title!r} orig_author_en={orig_author_en!r} kanji_map={kanji_map}", flush=True)
    if not orig_title or not orig_author_en:
        ...
        if not orig_author_en and nlk["orig_author"]:
            # 키릴 등 라틴 없는 원저자명은 700 도치 불가 → 버림
            if re.search(r"[A-Za-z]", nlk["orig_author"]):
                orig_author_en = nlk["orig_author"]
        # NLK 100/700 한자 저자명 → kanji_map 보완 (일본어 책)
        if nlk.get("orig_author_cjk") and primary_name and not kanji_map.get(primary_name):
            kanji_map[primary_name] = nlk["orig_author_cjk"]
...
def _marc_suffix_from_responsibility_pairs(pairs: list[tuple[str, str]]) -> str | None:
    ...
    for i, (role, names) in enumerate(groups):
        ...
        name_str = names[0] + "".join(f" ,$e {n}" for n in names[1:])
        if i == 0:
            parts.append(f"/$d {name_str} {verb}")
        else:
            parts.append(f" ;$e {name_str} {verb}")
    return "".join(parts)

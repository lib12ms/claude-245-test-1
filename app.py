...
def fetch_nl_library(isbn13: str) -> dict:
    ...
    알라딘에서 정보를 못 가져왔을 때 폴백으로 호출됩니다.

    1. ISBN 서지정보 API (seoji) → AUTHOR 필드에서 영문 저자명 추출
    2. 소장자료 API (search) → 원표제(원제) 필드 추출
    ...
    try:
        ...
        if docs:
            doc = docs[0]
            import sys
            print(f"[NL DEBUG] TITLE={doc.get('TITLE','')} AUTHOR={doc.get('AUTHOR','')}", file=sys.stderr)

            # 원표제: TITLE 필드에 "한글표제 = 원서명" 형태로 올 수 있음
            title_raw = doc.get("TITLE", "")
            if "=" in title_raw and not result["orig_title"]:
                candidate = title_raw.split("=", 1)[1].strip()
                # 괄호 안 연도 제거, 한글 없는 것만 채택
    ...
    try:
        ...
        for item in root.iter("item"):
            # 모든 텍스트를 합쳐서 원표제/원저자명 키워드 패턴 탐색
            # 주기사항(note, contents 등) 필드에
            # "원표제: 花に埋もれる" / "원저자명: 彩瀬まる" 형태로 들어있음
            all_text = " ".join(
                (el.text or "") for el in item.iter() if el.text
            )

            # 원표제 추출
            if not result["orig_title"]:
                m = re.search(r"원표제\s*[:：]\s*([^\s,;/]+(?:\s[^\s,;/]+)*)", all_text)
                if m:
                    candidate = m.group(1).strip()
                    if candidate and not re.search(r"[\uac00-\ud7a3]", candidate):
                        result["orig_title"] = remove_year(candidate)

            # 원저자명 추출 (영문/한자/가나 모두 허용)
            if not result["orig_author_en"]:
                m = re.search(r"원저자명\s*[:：]\s*([^\s,;/]+(?:\s[^\s,;/]+)*)", all_text)
                if m:
                    candidate = m.group(1).strip()
                    # 한글이 없는 이름만 채택 (영문 또는 한자/가나)
                    if candidate and not re.search(r"[\uac00-\ud7a3]", candidate):
                        result["orig_author_en"] = candidate

            # title 필드에 "한글표제 = 원서명" 형태도 보조로 확인
            if not result["orig_title"]:
                title_el = item.find("title")
                if title_el is not None and title_el.text and "=" in title_el.text:
                    candidate = title_el.text.split("=", 1)[1].strip()
                    if candidate and not re.search(r"[\uac00-\ud7a3]", candidate):
                        result["orig_title"] = remove_year(candidate)

            if result["orig_title"] or result["orig_author_en"]:
                break

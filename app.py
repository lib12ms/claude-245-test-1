...
[생성 필드]
  ...
  710 0_  기관명 부출기입
  900 10  원저자 한글명 부출 (번역·동아시아 원저 맥락에서는 245와 같은 한글 표기; 서양 원저만 성·이름 도치)
"""
...
def build_900(
    ...
    """
    - 번역서(246)·또는 동아시아·원표제 CJK/가나 맥락(japanese_family_book): 900은 245 책임과 같은 한글 표기만
      (이름 도치·한자 글자별 한국음역·가나 음역으로 500과 어긋나게 두지 않음).
    - 서양 원저만: 영문 원저자명이 있으면 한글 성·이름 도치, 없으면 한글 표기.
    """
...
def _isbn_lookup():
    ...
    japanese_family_900 = translation_book or jp_book_for_roman
    if field_500:

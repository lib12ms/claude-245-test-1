...
def fetch_nl_library(isbn13: str) -> dict:
    ...
    try:
        ...
        for item in root.iter("item"):
            # 원표제 필드: <titleInfo type="uniform"> 또는 <subTitle> 형태로 올 수 있음
            # 소장자료 API는 title 필드에 "한글표제 = 원표제" 형태로 오기도 함
            title_el = item.find("title")
            if title_el is not None and title_el.text:
                t = title_el.text.strip()
                # "한글 표제 = 원서명" 형태 → "=" 뒤를 원서명으로
                if "=" in t:
                    ...
                    if candidate and not re.search(r"[\uac00-\ud7a3]", candidate):
                        ...
                        break
            # titleOther 필드도 확인
            other_el = item.find("titleOther")
            if other_el is not None and other_el.text:
                candidate = other_el.text.strip()
                if candidate and not re.search(r"[\uac00-\ud7a3]", candidate):
                    result["orig_title"] = remove_year(candidate)
                    break
    except Exception:

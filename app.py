...
def _gpt_orig_info_lookup(
    ...
    def _one_attempt(attempt_no: int, msg: str) -> dict:
        try:
    ...
    result = _one_attempt(1, _build_user_msg(None, None))

    if not result["orig_title"] and not result["orig_author_en"]:
        # 둘 다 비었으면 동일 조건으로 1회 재시도 (검색·LLM 비결정성 완화)
        result = _one_attempt(2, _build_user_msg(None, None))
    elif result["orig_title"] and not result["orig_author_en"]:
        # 원서명만 찾음 → 그 원서명을 힌트로 원저자명을 2차 검색
        second = _one_attempt(2, _build_user_msg(result["orig_title"], None))
        ...
        # 원저자명만 찾음 → 그 저자명을 힌트로 원서명을 2차 검색
        second = _one_attempt(2, _build_user_msg(None, result["orig_author_en"]))

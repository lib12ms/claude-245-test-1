...
def enrich_kanji_map_from_aladin_blob(blob: str, names: list[str], km: dict) -> None:
    """페이지/HTML에서 '한글명（漢字）' 형태로 빠진 한자명 보완."""
    if not blob:
        return
    for nm in names:
        key = (nm or "").strip()
        if not key or km.get(key):
            continue
        m = re.search(re.escape(key) + r"\s*[（(]\s*([\u4e00-\u9fff\u30a0-\u30ff]+)\s*[）)]", blob)
        if m:
            km[key] = m.group(1).strip()
...
# ─────────────────────────────────────────────
# 국중 보강(확장 파싱) — Render 등 단일 파일 배포 시 import 오류 방지
# ─────────────────────────────────────────────
def _nl_sup_parse_title_field(title_raw: str) -> str | None:
    """서지 API TITLE 필드에서 '=' 뒤 원표제 후보."""
    if not title_raw or "=" not in title_raw:
        return None
    candidate = title_raw.split("=", 1)[1].strip()
    return remove_year(candidate) if candidate else None


def _nl_sup_pick_roman_author(author_raw: str) -> str | None:
    if not author_raw:
        return None
    for part in re.split(r"[;,]", author_raw):
        part = part.strip()
...
def _scrape_nl_detail_relaxed(control_no: str) -> dict[str, str | None]:
    """소장자료 상세 HTML — 원서명 라벨·한자 원표제 등 scrape_nl_detail보다 넓게 시도."""
    out: dict[str, str | None] = {"orig_title": None, "orig_author_en": None}
    if not control_no:
        return out
    url = (
    ...
    m = re.search(
        r"원저자명\s*[:：]\s*(.+?)(?=\s*원표제|\s*원서명|\s*내용|\s*일본어|\s*중국어|\s*영어|\s*ISBN|$)",
        text,
    )
    if m:
        candidate = re.sub(r"\s+", " ", m.group(1)).strip()
        if candidate and not re.search(r"[\uac00-\ud7a3]", candidate):
            out["orig_author_en"] = candidate

    hangul_only_title: str | None = None
    for label in ("원표제", "원서명"):
        m = re.search(
            rf"{label}\s*[:：]\s*(.+?)(?=\s*내용|\s*일본어|\s*중국어|\s*영어|\s*ISBN|\s*원저자|\s*원표제|\s*원서명|$)",
            text,
        )
        if not m:
            continue
        candidate = re.sub(r"\s+", " ", m.group(1)).strip()
        candidate = remove_year(candidate)
        if not candidate:
            continue
        if re.search(r"[\u3040-\u30ff\u4e00-\u9fffA-Za-z]", candidate):
            out["orig_title"] = candidate
            break
        if re.search(r"[\uac00-\ud7a3]", candidate) and hangul_only_title is None:
            hangul_only_title = candidate
    if out["orig_title"] is None and hangul_only_title:
        out["orig_title"] = hangul_only_title

    return out


def _seoji_lookup_relaxed(isbn13: str) -> dict[str, str | None]:
    r: dict[str, str | None] = {"orig_title": None, "orig_author_en": None}
    if not NL_API_KEY:
        return r
    try:
        resp = requests.get(
    ...
    cand = _nl_sup_parse_title_field(title_raw)
    if cand:
        cand = remove_year(cand)
        if re.search(r"[\u3040-\u30ff\u4e00-\u9fffA-Za-z]", cand):
            r["orig_title"] = cand
        elif re.search(r"[\uac00-\ud7a3]", cand) and not re.search(
            r"[\u3040-\u30ff\u4e00-\u9fff]", cand
        ):
            r["orig_title"] = cand
    author_raw = doc.get("AUTHOR") or ""
    oa = _nl_sup_pick_roman_author(author_raw)
    if oa:
        r["orig_author_en"] = oa
    return r


def _coll_control_no_relaxed(isbn13: str) -> str | None:
    if not NL_API_KEY:
        return None
    try:
        resp = requests.get(
...
def supplement_orig_metadata_from_nl(
    isbn13: str,
    orig_title: str | None,
    orig_author_en: str | None,
) -> tuple[str | None, str | None]:
    """알라딘·fetch_nl 이후에도 비어 있으면 국중으로 빈 슬롯만 채운다."""
    ot, oa = orig_title, orig_author_en
    if not NL_API_KEY:
        return ot, oa
    if ot and oa:
        return ot, oa
    ...
    if not ot or not oa:
        nl1 = _seoji_lookup_relaxed(isbn13)
        if not ot and nl1.get("orig_title"):
            ot = nl1["orig_title"]
        if not oa and nl1.get("orig_author_en"):
            oa = nl1["orig_author_en"]

    if not ot or not oa:
        cn = _coll_control_no_relaxed(isbn13)
        if cn:
            nl2 = _scrape_nl_detail_relaxed(cn)
            if not ot and nl2.get("orig_title"):
                ot = nl2["orig_title"]
            if not oa and nl2.get("orig_author_en"):
                oa = nl2["orig_author_en"]
...
def collect_orig_info(item: dict, item_id: str, isbn13: str, title: str, authors: list[dict]) -> dict:
    ...
    # 3b순위: 위에서도 비었을 때만 국중 재시도(확장 파싱·원서명 라벨)
    if not orig_title or not orig_author_en:
        ot_sup, oa_sup = supplement_orig_metadata_from_nl(isbn13, orig_title, orig_author_en)
        if not orig_title and ot_sup:
            orig_title = ot_sup
        if not orig_author_en and oa_sup:
            orig_author_en = oa_sup

...
검색 전략:
- 원서명·원저자명 모두 모를 때: "[번역서 제목] [저자 한글명] original title", "[번역서 제목] [저자 한글명] 원저자" 등으로 검색해 두 정보를 함께 확인하라.
- 원저자명만 모를 때: "[원서명] author", "[번역서 제목] [저자 한글명] original author" 순으로 검색하라.
- 원서명만 모를 때: "[번역서 제목] [저자 한글명] original title", "[원저자명] [번역서 제목]" 순으로 검색하라.
- 저자 정보에 한국어 이름(예: 도미닉 페트먼)이 있으면 그 음역에 해당하는 외국 저자를 식별해 검색에 활용하라.
- 동명의 한국어 번역서가 여러 권 있을 수 있으므로, 저자 정보를 반드시 활용해 올바른 책을 특정하라.
규칙:
- 웹 검색으로 확인된 사실만. 불확실하면 null.
- orig_title과 orig_author는 반드시 같은 책에 대한 정보여야 한다. 원서명을 찾았더라도 해당 책의 저자가 입력된 저자 정보(한글명 음역 포함)와 일치하지 않으면 orig_title도 null로 반환하라.
...
def collect_orig_info(item: dict, item_id: str, isbn13: str, title: str, authors: list[dict]) -> dict:
    ...
    if not orig_title or not orig_author_en:
        ...
        gpt = _gpt_orig_info_lookup(title, authors, known_orig_author=orig_author_en, known_orig_title=_reliable_orig_title, publisher=_pub, pub_year=_year)
...
def decide_name_order_via_llm(hangul_name: str, context: str = "") -> dict:
    ...
    if not _GEMINI_AVAILABLE or not GEMINI_API_KEY:
        # 스페인/중남미 복합 성씨: 첫 단어가 스페인계 이름이고 두 번째가 이름 아님
        # 가브리엘 가르시아 마르케스 → 가르시아 마르케스, 가브리엘
        ...
        # 한글 음역 외국 저자명: 마지막 단어를 성씨로 도치 (러시아·서양 이름 공통)
    ...
    try:
        ...
        if context:
            user_msg += f"\n\ucee8\ud14d\uc2a4\ud2b8: {context}"
        response = model.generate_content(_NAME_ORDER_SYSTEM_PROMPT + "\n\n" + user_msg)
...
def _isbn_lookup():
    ...
    # 알라딘이 역자를 누락한 경우 대비:
    # 저자 이름이 한국 성씨로 시작하지 않으면 외국어 음차 이름으로 간주해 번역서 처리
    def _author_is_foreign_transliteration(name: str) -> bool:
        name = name.strip()
        if not name or not re.search(r"[가-힣]", name):
            return False
        return not is_korean_person_by_surname(name)

    non_trans_authors = [a for a in authors if not a["is_org"] and a["role"] not in TRANS_ROLES]
    has_foreign_author = any(_author_is_foreign_transliteration(a["name"]) for a in non_trans_authors)
    needs_orig_lookup  = has_translator or has_foreign_author
    ...
    translation_book = has_translator or has_foreign_author
    ...
    if western_700_english:
        ...
        for a in persons_for_700:
            ...
            if (
                en_prof
                and re.search(r"[A-Za-z]", en_prof)
                and not (ai_match or {}).get("is_east_asian")
                and not orig_title_has_cjk_or_kana
                and not (ai_match or {}).get("kanji_name")
            ):

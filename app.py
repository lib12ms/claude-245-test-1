...
from korean_given_names import is_registered_korean_given_name, korean_given_names
...
def _korean_given_not_in_birth_registry(
    ...
    *,
    ...
    """
    형식상 한글 개인명(성+이름 1~2음절)인데 출생신고 이름 통계에 없음 → 필명·비정상 의심.
    ...
    if not given or len(given) > 2:
        return False
    return not is_registered_korean_given_name(given)
...
def _korean_hangul_short_given_exception(name: str) -> bool:
    ...
    if not given or len(given) > 2:
        return False
    if korean_given_names() and not is_registered_korean_given_name(given):
        return False
    return True
...
def marc700_second_indicator(
    ...
    *,
    ...
    한국인 한글명: 성 제외 3음절 이상 → 0. 미등록 이름(출생통계) → 0.
    1~2음절 structured 예외(김강 등, 이름 DB 등록) → 1.
    """
    n = (name or "").strip()
    if not n:
        return "1"

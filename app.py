...
NLK_API_KEY = os.environ.get("NLK_API_KEY", "").strip()
...
def extract_title_statement_from_nlk_text(text: str) -> str | None:
    """
    국중 표제/저자사항 앞부분만 (책임표시 '/' 이전).
    예: 카프네 = Cafuné : 아베 아키코 장편소설
    """
    t = re.sub(r"\s+", " ", (text or "").strip())
    if not t or len(t) < 3:
        return None
...
def fetch_nlk_bib_by_isbn(isbn13: str) -> dict[str, str | None]:
    """
    국립중앙도서관 Open API로 ISBN 서지 조회.
    NLK_API_KEY 필요. 반환: responsibility, orig_title, title_for_245
    """
    ...
    resp_candidates: list[str] = []
    orig_candidates: list[str] = []
    title_candidates: list[str] = []
    blobs: list[str] = []

    def _scan_text(s: str) -> None:
        if not s:
            return
        hit = extract_responsibility_from_nlk_bib_text(s)
        if hit:
            resp_candidates.append(hit)
        ...
        ts = extract_title_statement_from_nlk_text(s)
        if ts:
            title_candidates.append(ts)

    def _try_holdings() -> None:
        params = {
            "key": key,
            "apiType": "json",
            "searchType": "isbn",
    ...
    def _try_seoji() -> None:
        ...
        for row in rows:
            if not isinstance(row, dict):
                continue
            for fld in ("TITLE", "title", "AUTHOR", "author"):
                val = row.get(fld)
                if val:
    ...
    responsibility = (
        max(resp_candidates, key=lambda s: (s.count(";"), len(s)))
        if resp_candidates
        else None
    )
    orig_title: str | None = None
    if orig_candidates:
        cjk = [
            c for c in orig_candidates
            if re.search(r"[\u3040-\u30ff\u4e00-\u9fff\u3400-\u4dbf]", c)
        ]
        pool = cjk if cjk else orig_candidates
        orig_title = remove_year(min(pool, key=len))
    title_for_245 = (
        max(title_candidates, key=len) if title_candidates else None
    )
    return {
        "responsibility": responsibility,
        "orig_title": orig_title,
        "title_for_245": title_for_245,
    }


def fetch_nlk_responsibility_statement(isbn13: str) -> str | None:
    """국중 책임표시만 필요할 때 (하위 호환)."""
    return fetch_nlk_bib_by_isbn(isbn13).get("responsibility")
...
def _isbn_lookup():
    ...
    nlk_bib = fetch_nlk_bib_by_isbn(isbn13)

    if has_translator:
        orig_info      = collect_orig_info(item, item_id, isbn13, title, authors)
        orig_title     = orig_info["orig_title"]
        if not orig_title and nlk_bib.get("orig_title"):
            orig_title = nlk_bib["orig_title"]
    ...
    # MARC 필드 생성 — 국중: 책임표시·원표제·표제(알라딘에 원서명 없을 때 보강)
    title_for_245 = title
    subtitle_for_245 = subtitle
    if nlk_bib.get("title_for_245"):
        title_for_245 = nlk_bib["title_for_245"]
        subtitle_for_245 = ""
    field_245 = build_245(
        title_for_245,
        subtitle_for_245,
        authors,
        responsibility=nlk_bib.get("responsibility"),
    )

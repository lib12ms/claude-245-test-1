...
[데이터 수집 순서]
  ...
  4. VIAF AutoSuggest (원저자 영문명이 없을 때) → 세계 국립도서관 전거 파일
...
def extract_english_author_after_korean_paren(text: str, limit: int = 16000) -> str | None:
    ...
    for m in re.finditer(
        r"[\uac00-\ud7a3][\uac00-\ud7a3\s]*\(([A-Z\u00c0-\u00d6\u00d8-\u00d6][A-Za-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff]+(?:\s+[A-Za-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff.\-\']+){1,})\)",
        chunk,
    ):
        candidate = re.sub(r"\s+", " ", m.group(1)).strip()
        words = candidate.split()
        if len(words) >= 2 and all(re.match(r"^[A-Za-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff.\-\']+$", w) for w in words):
            return candidate
    return None
...
_INTRO_KO_EN_ROLE_RE = re.compile(
    r"(.{2,80}?) \(([A-Za-zÀ-ÖØ-öø-ÿ][A-Za-z0-9À-ÖØ-öø-ÿ ,.'\-]{1,120})\) \((지은이|저자|글|엮은이|그린이|원작|편저|감수)\)"
)
...
def parse_intro_author_english_pairs(soup: BeautifulSoup, page_text: str) -> list[dict]:
    ...
    for m in _INTRO_KO_EN_ROLE_RE.finditer(one):
        ...
        if len(ko) < 2 or len(en) < 2:
            continue
        if not re.fullmatch(r"[A-Za-zÀ-ÖØ-öø-ÿ0-9 ,.'\-\s]+", en):
            continue
...
def _merge_searchword_initials(parts: list[str]) -> list[str]:
    ...
    i = 0
    while i < len(parts):
        letters = re.sub(r"[^A-Za-zÀ-ÖØ-öø-ÿ]", "", parts[i])
        if letters.isupper() and len(letters) == 1:
            initials: list[str] = []
            j = i
            while j < len(parts):
                lj = re.sub(r"[^A-Za-zÀ-ÖØ-öø-ÿ]", "", parts[j])
...
def _titleize_foreign_caps_token(tok: str) -> str:
    ...
    core = re.sub(r"[^A-Za-zÀ-ÖØ-öø-ÿ]", "", t)
...
def scrape_aladin_product(item_id: str, orig_title_hint: str | None = None) -> dict:
        ...
        if m:
            ...
            if hint:
                ...
                author_parts = [
                    ...
                    and len(re.sub(r"[^A-Za-zÀ-ÖØ-öø-ÿ]", "", p)) > 1
                ]
    ...
    if not result["orig_author_en"]:
        ...
        for m in re.finditer(
            r"[\uac00-\ud7a3][\uac00-\ud7a3\s]+\(([A-Z\u00c0-\u00d6][A-Za-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff]+(?:\s+[A-Za-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff]+){1,})\)",
            search_text
        ):
            candidate = re.sub(r"\s+", " ", m.group(1)).strip()
            words = candidate.split()
    ...
    for meta in soup.find_all("meta"):
        ...
        if not result["orig_author_en"]:
            for p in content.split(","):
                p = p.strip()
                ws = p.split()
                if len(ws) >= 2 and all(re.match(r"^[A-Za-zÀ-ÖØ-öø-ÿ.\-\']+$", w) for w in ws):
...
# 원제·원저자 수집 메인 로직
# ─────────────────────────────────────────────
def fetch_viaf_orig_author(name_ko: str) -> str | None:
    """
    한글 저자명으로 VIAF AutoSuggest 검색 → 원어 공인형 이름 반환.
    VIAF: 세계 국립도서관 전거 파일 통합 서비스 (무료, 인증 불필요).
    반환값은 "Georges Bernanos" / "Haruki Murakami" 형태 (도치 전).
    """
    ...
    try:
        ...
        for r in results[:3]:
            if r.get("nametype") != "personal":
                continue
            display = (r.get("displayForm") or "").strip()
            if not display or re.search(r"[가-힣]", display):
                continue
            # VIAF는 "Last, First" 형식으로 반환 → "First Last"로 변환
            if "," in display:
...
def collect_orig_info(item: dict, item_id: str, isbn13: str, title: str, authors: list[dict]) -> dict:
    ...
    if not orig_title or not orig_author_en:
        nlk = fetch_nlk_orig_info(isbn13)
    ...
    # 4순위: VIAF 폴백 — 원저자 영문명이 없으면 한글 저자명으로 VIAF 검색
    if not orig_author_en and primary_name:
        orig_author_en = fetch_viaf_orig_author(primary_name)
...
def decide_name_order_via_llm(hangul_name: str, context: str = "") -> dict:
    ...
    if not _GEMINI_AVAILABLE or not GEMINI_API_KEY:
        if len(parts) == 2:
            return {"action": "REORDER", "result": f"{parts[-1]}, {parts[0]}", "reason": "fallback-2parts"}

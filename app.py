...
def _nlk_api_key() -> str:
    ...
    return (os.environ.get("NLK_API_KEY") or os.environ.get("NL_API_KEY") or "").strip()
...
def _nlk_seoji_orig_candidates(isbn13: str, key: str, timeout: float) -> list[str]:
    """ISBN 서지 API — TITLE·AUTHOR에서 원표제 후보만 수집."""
    out: list[str] = []
    params = {
        "cert_key": key,
        "result_style": "json",
    ...
    if (data.get("RESULT") or "").upper() == "ERROR":
        return out
    rows = data.get("docs") or data.get("DOC") or []
    ...
    for row in rows:
        ...
        if title_raw:
            ...
            if hit:
                out.append(hit)
        for fld in ("AUTHOR", "author"):
            val = row.get(fld)
            if val:
                hit = extract_orig_title_from_nlk_text(str(val))
                if hit:
                    out.append(hit)
    return out


def fetch_nlk_orig_title_by_isbn(isbn13: str) -> str | None:
    """
    ...
    orig_candidates = _nlk_seoji_orig_candidates(
        isbn13, key, timeout=min(8, max(3, _left()))
    )
...
def _isbn_lookup():
    ...
    if has_translator:
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            ...
            if _nlk_api_key():
                nlk_fut = pool.submit(fetch_nlk_orig_title_by_isbn, isbn13)

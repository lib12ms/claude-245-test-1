...
- **같은 저자가 여러 책을 썼을 수 있다.** 저자만 일치해도 다른 책일 수 있으니, 부제가 주어졌으면
  그 부제의 의미·키워드가 원서의 부제(또는 핵심 주제)와 실제로 대응하는지 반드시 확인하라.
  제목만 보고 저자의 가장 유명한/대표적인 책을 추측해서 반환하지 말 것 — 부제 의미가 다르면 다른 책이다.
규칙:
...
- 부제가 주어졌는데 찾은 원서의 부제·주제가 한국어판 부제와 명백히 다르면(저자가 같아도) orig_title을 null로 반환하라.
...
# 원제·원저자 수집 메인 로직
# ─────────────────────────────────────────────
def collect_orig_info(item: dict, item_id: str, isbn13: str, title: str, authors: list[dict], subtitle: str | None = None) -> dict:
    ...
    if not orig_title or not orig_author_en:
        ...
        gpt = _gpt_orig_info_lookup(title, authors, known_orig_author=orig_author_en, known_orig_title=_reliable_orig_title, publisher=_pub, pub_year=_year, subtitle=subtitle)
...
def _isbn_lookup():
    ...
    if needs_orig_lookup:
        orig_info = collect_orig_info(item, item_id, isbn13, title, authors, subtitle=subtitle)

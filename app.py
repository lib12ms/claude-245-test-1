...
try:
    ...
    _lang_field_mod  = _importlib.import_module("041")
    _LangFieldBuilder = _lang_field_mod.LangFieldBuilder
    _LFB_AVAILABLE   = True
except Exception:
    _LangFieldBuilder = None
    _LFB_AVAILABLE   = False

from name_order_by_lang import extract_lang_h as _extract_lang_h

_lang_builder_instance = None  # LangFieldBuilder 싱글턴 (최초 호출 시 초기화)

def _get_lang_builder():
    global _lang_builder_instance
    if _lang_builder_instance is None and _LFB_AVAILABLE and _OPENAI_AVAILABLE and OPENAI_API_KEY:
        _lang_builder_instance = _LangFieldBuilder(
            openai_client=_openai_module.OpenAI(api_key=OPENAI_API_KEY),
        )
    return _lang_builder_instance
...
# (예: "하와이 대저택" — 실존 외국인 음역이 아니라 한국 유튜버의 채널명 필명, 2026-06-29 확인)
KNOWN_PEN_NAMES = frozenset({"하와이 대저택", "다숲", "느린호수"})

# inferred_country(GPT가 반환한 한국어 국가명) 기반 도치 결정
_WESTERN_COUNTRIES: frozenset[str] = frozenset({
    "미국", "영국", "프랑스", "독일", "이탈리아", "스페인", "포르투갈", "러시아",
    "네덜란드", "스웨덴", "노르웨이", "덴마크", "핀란드", "벨기에", "스위스",
    "오스트리아", "폴란드", "체코", "헝가리", "루마니아", "그리스",
    "이스라엘", "호주", "캐나다", "아일랜드", "아르헨티나",
    "브라질", "멕시코", "칠레", "콜롬비아", "페루", "쿠바", "우크라이나",
    "세르비아", "크로아티아", "슬로베니아", "불가리아", "슬로바키아",
})
_NO_INVERT_COUNTRIES: frozenset[str] = frozenset({
    "일본", "중국", "한국", "대만", "홍콩", "싱가포르", "베트남", "북한",
})
...
def korean_name_reverse(name: str) -> str | None:
    ...
    if len(parts) >= 3:
        ...
        if first in _SPANISH_GIVEN_NAMES_KOREAN and second not in _SPANISH_GIVEN_NAMES_KOREAN:
...
_SPANISH_GIVEN_NAMES_LATIN = frozenset({
    ...
    "Pedro", "Alfredo", "Rafael", "Fernando", "Federico", "Eduardo", "Antonio", "José", "Jose",
    ...
    "Óscar", "Oscar", "Guillermo", "Horacio", "Hugo", "Lorenzo", "Felipe",
    "Santiago", "Gonzalo", "Héctor", "Hector", "Raúl", "Raul", "Víctor", "Victor",
    "Emilio", "Marco", "Agustín", "Agustin", "Joaquín", "Joaquin", "Gerardo",
    "Leopoldo", "León", "Leon", "Luciano", "Marcos",
    # 남성 (확장)
    "Alonso", "Ángel", "Angel", "Emiliano", "Esteban", "Ezequiel", "Fabián", "Fabian",
    "Jesús", "Jesus", "Leonardo", "Lucas", "Mateo", "Máximo", "Maximo", "Pascual",
    "Ramiro", "Rodolfo", "Rosendo", "Salomón", "Salomon", "Tomás", "Tomas",
    "Valentín", "Valentin", "Vicente", "Xavier", "Amado", "Anselmo", "Bernardo",
    "Cristóbal", "Cristobal", "Domingo", "Elías", "Elias", "Fabricio", "Feliciano",
    "Gaspar", "Genaro", "Gilberto", "Hilario", "Homero", "Ismael", "Jaime",
    "Jerónimo", "Jeronimo", "Leandro", "Moisés", "Moises", "Osvaldo", "Reinaldo",
    "Rigoberto", "Rolando", "Samuel", "Saúl", "Saul", "Teodoro", "Wilfredo",
    "Zacarías", "Zacarias",
    # 포르투갈어(브라질)
    "Paulo", "João", "Joao", "Raduan", "Clarice", "Lya",
    # 여성
    ...
    "Rosario", "Ángela", "Angela", "Adriana", "Gabriela", "Fernanda",
    "Susana", "Verónica", "Veronica", "Inés", "Ines",
    # 여성 (확장)
    "Alejandra", "Alba", "Amalia", "Amelia", "Antonia", "Aurora", "Belén", "Belen",
    "Blanca", "Camila", "Carolina", "Celia", "Concepción", "Concepcion", "Constanza",
    "Delia", "Diana", "Elisa", "Esperanza", "Estela", "Eugenia", "Eva", "Flor",
    "Francisca", "Graciela", "Guadalupe", "Hilda", "Irene", "Josefina", "Leticia",
    "Lidia", "Lourdes", "Luz", "Manuela", "Marcela", "Marisol", "Marta", "Natalia",
    "Nora", "Norma", "Ofelia", "Olga", "Paloma", "Paula", "Raquel", "Regina",
    "Renata", "Roberta", "Rocío", "Rocio", "Sara", "Soledad", "Valentina",
    "Victoria", "Ximena", "Yolanda",
})
...
_SPANISH_GIVEN_NAMES_KOREAN = frozenset({
    ...
    "페드로", "알프레도", "라파엘", "페르난도", "페데리코", "에두아르도", "안토니오", "호세",
    ...
    "하비에르", "세르히오", "세사르", "옥타비오",
    "오스카르", "기예르모", "오라시오", "우고", "로렌소", "펠리페",
    "산티아고", "곤살로", "엑토르", "라울", "빅토르", "에밀리오", "마르코",
    "아구스틴", "호아킨", "헤라르도", "레오폴도", "레온", "루시아노", "마르코스",
    # 남성 (확장)
    "알론소", "앙헬", "에밀리아노", "에스테반", "에세키엘", "파비안",
    "헤수스", "레오나르도", "루카스", "마테오", "막시모", "파스쿠알",
    "라미로", "로돌포", "로센도", "살로몬", "토마스",
    "발렌틴", "비센테", "하비에르", "아마도", "안셀모", "베르나르도",
    "크리스토발", "도밍고", "엘리아스", "파브리시오", "펠리시아노",
    "가스파르", "헤나로", "힐베르토", "힐라리오", "오메로", "이스마엘", "하이메",
    "헤로니모", "레안드로", "모이세스", "오스발도", "레이날도",
    "리고베르토", "롤란도", "사무엘", "사울", "테오도로", "윌프레도",
    "사카리아스",
    # 포르투갈어(브라질)
    "파울루", "주앙", "라두안", "클라리시", "리아",
    # 여성
    ...
    "로사리오", "앙헬라", "아드리아나", "가브리엘라", "페르난다",
    "수사나", "베로니카", "이네스",
    # 여성 (확장)
    "알레한드라", "알바", "아말리아", "아멜리아", "안토니아", "아우로라", "벨렌",
    "블랑카", "카밀라", "카롤리나", "셀리아", "콘셉시온", "콘스탄사",
    "델리아", "디아나", "엘리사", "에스페란사", "에스텔라", "에우헤니아", "에바", "플로르",
    "프란시스카", "그라시엘라", "과달루페", "일다", "이레네", "호세피나", "레티시아",
    "리디아", "루르데스", "루스", "마누엘라", "마르셀라", "마리솔", "마르타", "나탈리아",
    "노라", "노르마", "오펠리아", "올가", "팔로마", "파울라", "라켈", "레히나",
    "레나타", "로베르타", "로시오", "사라", "솔레다드", "발렌티나",
    "빅토리아", "히메나", "욜란다",
})
...
_KANJI_RUN_RE = re.compile(r"[\u4e00-\u9fff\u3400-\u4dbf]+|[\u3040-\u30ff]+")


def _jp_mixed_name_reading_hangul(script: str) -> str | None:
    """
    \ud55c\uc790+\uac00\ub098 \ud63c\ud569 \uc77c\ubcf8 \uc6d0\uc800\uc790\uba85 \u2192 \uad6d\uc911 \uad00\ub840\ub300\ub85c \ubcc0\ud658: \ud55c\uc790 \uad6c\uac04\uc740 \ud55c\uad6d \ud55c\uc790\uc74c,
    \uac00\ub098 \uad6c\uac04\uc740 \ubc1c\uc74c \uadf8\ub300\ub85c \ud55c\uae00 \uc74c\uc5ed, \uad6c\uac04\ubcc4\ub85c \uc774\uc5b4\ubd99\uc784(\uacf5\ubc31 \uc5c6\uc74c).
    \uc608: \u4e0a\u91ce\u305d\u3089 \u2192 \uc0c1\uc57c(\ud55c\uc790\uc74c) + \uc18c\ub77c(\uac00\ub098 \uc74c\uc5ed) = \uc0c1\uc57c\uc18c\ub77c
    """
    s = re.sub(r"\s+", "", (script or "").strip())
    if not s:
        return None
    runs = _KANJI_RUN_RE.findall(s)
    if not runs or "".join(runs) != s:
        return None  # \ud55c\uc790\u00b7\uac00\ub098 \uc774\uc678 \ubb38\uc790\uac00 \uc11e\uc5ec\uc788\uc73c\uba74 \uc2e0\ub8b0 \uc5b4\ub824\uc6c0
    out_parts: list[str] = []
    for run in runs:
        if re.fullmatch(r"[\u4e00-\u9fff\u3400-\u4dbf]+", run):
            reading = kanji_to_korean_reading(run)
            if not reading:
                return None
            out_parts.append(reading)
        else:
            kata = jaconv.hira2kata(run) if _PYKAKASI_JACONV else run
            conv = _katakana_to_hangul(kata)
            if not conv:
                return None
...
def _extract_source_urls(response) -> list[str]:
    ...
    response.output_text만 쓰면 버려지는 url_citation annotation을 직접 순회해서 추출."""
    urls: list[str] = []
    try:
...
def _gpt_orig_info_lookup(
    ...
    def _one_attempt(attempt_no: int, msg: str, *, use_web_search: bool = True) -> dict:
        try:
            client = _openai_module.OpenAI(api_key=OPENAI_API_KEY, timeout=60.0)
            kwargs: dict = dict(
                model="gpt-4o",
                input=[
            ...
            if urls:
                print(f"[gpt_orig_info_lookup] attempt {attempt_no} 참고 URL: {urls}")
            ...
            print(f"[gpt_orig_info_lookup] attempt {attempt_no} (web={use_web_search}): title={orig_title!r} author={orig_author_en!r}")
            return {"orig_title": orig_title, "orig_author_en": orig_author_en, "orig_author_kanji": orig_author_kanji}
        except Exception as e:
            print(f"[gpt_orig_info_lookup] attempt {attempt_no} 실패: {type(e).__name__}: {e}")
            return {"orig_title": None, "orig_author_en": None, "orig_author_kanji": None}

    # 1차: 웹 검색 없이 훈련 데이터만으로 시도 (웹 검색이 오히려 오답 유발하는 경우 방지)
    result = _one_attempt(1, _build_user_msg(None, None), use_web_search=False)

    if not result["orig_title"] and not result["orig_author_en"]:
        # 훈련 데이터로 못 찾음 → 웹 검색으로 재시도
        result = _one_attempt(2, _build_user_msg(None, None), use_web_search=True)

    if not result["orig_title"] and not result["orig_author_en"]:
        # 웹 검색으로도 둘 다 없으면 1회 더 재시도
        result = _one_attempt(3, _build_user_msg(None, None), use_web_search=True)
    elif result["orig_title"] and not result["orig_author_en"]:
        # 원서명만 찾음 → 그 원서명을 힌트로 원저자명을 웹 검색
        second = _one_attempt(3, _build_user_msg(result["orig_title"], None), use_web_search=True)
        ...
        # 원저자명만 찾음 → 그 저자명을 힌트로 원서명을 웹 검색
        second = _one_attempt(3, _build_user_msg(None, result["orig_author_en"]), use_web_search=True)
...
def _book_description_for_gpt_hint(item: dict, limit: int = 500) -> str | None:
    ...
    원서 시리즈명·임프린트명 등 GPT 검색에 도움될 단서가 섞여 있는 경우가 많음."""
    for key in ("fullDescription2", "fullDescription", "description"):
        val = item.get(key)
        if isinstance(val, str) and val.strip():
...
def collect_orig_info(item: dict, item_id: str, isbn13: str, title: str, authors: list[dict], subtitle: str | None = None, category: str | None = None) -> dict:
    ...
    if isinstance(sub_info, dict):
        ...
        if api_orig:
            ...
            print(f"[collect_orig_info] aladin_api originalTitle={api_orig!r}")
    ...
    if not orig_title or not orig_author_en:
        ...
        if not orig_title and nlk["orig_title"]:
            # NLK가 원저자도 반환했는데 이미 알고 있는 orig_author_en과 이름 토큰이 전혀 안 겹치면
            # NLK가 잘못된 ISBN 매핑 데이터를 반환한 것 → orig_title 거부
            ...
            if orig_author_en and _nlk_author and re.search(r"[A-Za-z]", _nlk_author):
                ...
                if _known_tok and _nlk_tok and not (_known_tok & _nlk_tok):
                    print(f"[collect_orig_info] NLK orig_title 거부 — 저자 불일치 (known={orig_author_en!r}, nlk={_nlk_author!r})")
                    _reject_nlk = True
            if not _reject_nlk:
    ...
    if not orig_title or not orig_author_en:
        ...
        print(f"[collect_orig_info] GPT 호출 직전 — title={title!r} orig_title={orig_title!r} orig_author_en={orig_author_en!r} desc_hint={_desc_hint!r}")
        gpt = _gpt_orig_info_lookup(title, authors, known_orig_author=orig_author_en, known_orig_title=_reliable_orig_title, publisher=_pub, pub_year=_year, subtitle=subtitle, category=category, author_kanji=_author_kanji, description_hint=_desc_hint)
        ...
        # GPT가 같은 호출에서 저자 한자명도 같이 찾았으면 바로 사용 (추가 비용 없음 — 이미 한 검색 결과 재활용).
        ...
        # GPT가 저자 한자명까지는 못 찾았으면, 새로 찾은 원서명(한자/가나)으로 알라딘 외서 검색을 한 번 더 시도.
        # enrich_kanji_map_from_foreign_title_search는 위 GPT 호출보다 먼저 끝나버려서
        # GPT가 찾아낸 원서명을 반영할 기회가 없었음 — 이제라도 그 원서명으로 시도 (알라딘 API라 GPT 비용 추가 없음).
        if (
            _orig_title_src == "gpt"
            and orig_title
            and re.search(r"[぀-ヿ一-鿿]", orig_title)
            ...
            enrich_kanji_map_from_foreign_title_search(orig_title, [a["name"] for a in non_trans], kanji_map)
...
def _responsibility_pairs_from_authors(authors: list[dict]) -> list[tuple[str, str]]:
    """책임표시용 (원본역할, 이름) — 개인·기관 저자 모두, 알라딘 등장 순서."""
...
# 숫자+영문자 조합(3D, 4K 등)에서 숫자를 영어식으로 읽는 경우
    ...
    for i in range(len(groups) - 1, -1, -1):
        ...
        for j in range(3, -1, -1):
            ...
            if d == 0:
                continue
            if d == 1 and j > 0:
                group += _KO_PLACES[j]  # 일십 → 십, 일백 → 백, 일천 → 천
            else:
...
def build_940(title: str) -> str | None:
    ...
    - 순수 숫자: 한국어 수 읽기 (64 → 육십사)
    - 숫자+영문 붙음 (3D, 4K): 영어식 숫자 + 영문자 발음 (쓰리디, 포케이)
    - 영문자: 영문자 발음 (AI → 에이아이)
    없으면 None."""
    t = (title or "").strip()
    if not t:
...
def build_700(author: dict, *, context: str = "", inferred_country: str = "") -> str:
    name = personal_name_for_700_field(author["name"].strip())
    if name in KNOWN_PEN_NAMES:
        return f"$a {name}"

    _country = (inferred_country or "").strip()

    # inferred_country \uae30\ubc18 \ub3c4\uce58 \uacb0\uc815 (\ud06c\ub864\ub9c1 \uad6d\uc801 \uc6b0\uc120)
    if _country in _WESTERN_COUNTRIES:
        if re.search(r"[A-Za-z]", name) and not re.search(r"[\uac00-\ud7a3]", name):
            name = english_name_reverse(name)
        elif re.search(r"[\uac00-\ud7a3]", name):
            rev = korean_name_reverse(name)
            if rev:
                name = rev
        return f"$a {name}"
    if _country in _NO_INVERT_COUNTRIES:
        return f"$a {name}"

    # inferred_country \uc5c6\uac70\ub098 \ubbf8\ub4f1\ub85d \uad6d\uac00 \u2192 \uae30\uc874 \ud3f4\ubc31
    if re.search(r"[A-Za-z]", name) and not re.search(r"[\uac00-\ud7a3]", name):
        name = english_name_reverse(name)
...
def marc700_second_indicator(
    ...
    *,
    ...
    팔리어·산스크리트어 원저자는 성씨 구분 없는 단일 이름 → 0.
    """
    n = (name or "").strip()
    if not n:
        return "1"
    ...
    if pali_sanskrit_book and role not in TRANS_ROLES:
        return "0"
    # 크롤링 국적이 서양 국가이고 번역자가 아닌 원저자 → 성씨 구조 확정 → 1_
    if (inferred_country or "").strip() in _WESTERN_COUNTRIES and role not in TRANS_ROLES:
        return "1"
    ai = author_info or []
    _short_exc = _korean_hangul_short_given_exception(n)
    if _short_exc:
        _by_sur = is_korean_person_by_surname(n)
        _given  = _extract_korean_given_part(n)
        print(f"[marc700_ind] short_exc=True name={n!r} by_surname={_by_sur} given={_given!r}")
    if (
        _short_exc
    ...
    if re.search(r"[가-힣]", n):
        ...
        # 옮긴이 본인 명의(알베르토 몬디 등)는 collect_orig_info가 author_info에 안 담음(원저자 전용
        # 정보라 옮긴이는 처음부터 제외) → author_info 매칭으로는 절대 못 잡힘. 번역서의 옮긴이인데
        # 한국 성씨가 아니면 필명이 아니라 외국인 본명 음역으로 보고 동일하게 1로 처리.
        ...
        if _korean_given_not_in_birth_registry(
            ...
            return "0"
        _structured = korean_personal_name_is_structured(n)
        _surname_ok = is_korean_person_by_surname(n)
        print(f"[marc700_ind] name={n!r} structured={_structured} surname={_surname_ok} → {'1' if _structured else '0'}")
        return "1" if _structured else "0"
    return "1"
...
def _isbn_lookup():
    ...
    # 옮긴이 역할이 명시되지 않은 번역서 대응(예: 원저자 "지음" + 출판사 편집부 "엮음"으로만
    # 표시되고 옮긴이 크레딧이 따로 없는 경우) — 주역할(지은이 등) 저자가 한국 성씨가 아니면
    # 번역서로 간주. 2026-06-25 발견: "이시바시 다케후미 (지은이), 혜화1117 편집부 (엮은이),
    # 로버트 파우저, 다카세 미나 (감수)"처럼 옮긴이가 없으면 has_translator=False가 되어
    # 원저자 한자명 조회도 안 되고 700 지시기호도 한국인 성씨 사전으로 잘못 떨어졌었음.
    _category_name = (item.get("categoryName") or "").strip()
    # 알라딘 카테고리에 "국내"가 있으면 한국 저자 책 → 성씨 사전 불일치를 외국인으로 오판 방지
    # 옮긴이·번역자가 없으면 한국 저자 책으로 간주. 옮긴이 없는 외국인 원저(bug #52 패턴)는
    # 드문 경우라 필명 오탐(느린호수 등)보다 피해가 작음.
    has_foreign_primary_author = False

    # 옮긴이가 있거나 주저자가 외국인이면 번역서(외국 저자), 아니면 한국 저자로 간주
    needs_orig_lookup = has_translator or has_foreign_primary_author
    ...
    # 041/546 필드 생성 (LangFieldBuilder)
    tag_041 = tag_546 = inferred_country = lang_h = None
    _builder = _get_lang_builder()
    if _builder:
        try:
            _detail = {"category_text": _category_name, "original_title": orig_title or ""}
            tag_041, tag_546, _ot41, inferred_country = _builder.get_kormarc_tags(item, _detail)
            if tag_041 and not tag_041.startswith("📕"):
                lang_h = _extract_lang_h(tag_041)
                print(f"[041] tag_041={tag_041!r} | tag_546={tag_546!r} | lang_h={lang_h!r} | country={inferred_country!r}")
            else:
                tag_041 = tag_546 = None
        except Exception as _e:
            print(f"[LangFieldBuilder] 오류: {_e}")
            tag_041 = tag_546 = None

    # lang_h가 외국어면 → 번역서로 보정 + 아직 미수집이면 원제·원저자 지연 수집
    # (옮긴이 없는 서양 원작: has_translator=False라도 041이 외국어 감지하면 번역서로 간주)
    _lang_h_is_foreign = bool(lang_h and lang_h not in ("kor", "und"))
    if _lang_h_is_foreign and not needs_orig_lookup:
        if not orig_title or not orig_author_en or not author_info:
            print(f"[041→orig] lang_h={lang_h!r} 외국어 감지 → 원제·원저자 지연 수집…")
            _oinfo2 = collect_orig_info(item, item_id, isbn13, title, authors, subtitle=subtitle, category=_category_name)
            if not orig_title:
                orig_title         = _oinfo2["orig_title"]
            if not orig_author_en:
                orig_author_en     = _oinfo2["orig_author_en"]
            if not author_info:
                author_info        = _oinfo2["author_info"]
            if not intro_persons:
                intro_persons      = _oinfo2.get("intro_persons") or []
            if not intro_author_pairs:
                intro_author_pairs = _oinfo2.get("intro_author_pairs") or []

    # MARC 필드 생성
    field_245 = build_245(title, subtitle, authors, item=item)
    field_940 = build_940(title)

    translation_book = has_translator or has_foreign_primary_author or _lang_h_is_foreign
    ...
    # 팔리어·산스크리트어 원저: 원저자명에 장모음 기호 등 특유의 발음 구별 부호가 있거나
    # 원제가 데바나가리 문자인 경우 → 원저자 700은 성씨 없는 단일 이름 구조(700 0_)
        ...
        if not orig_names_500 and orig_author_en and not jp_ctx_for_500:
            ...
            if len(chunks) >= 2 and author_info:
                ...
                if (
                    ...
                    # 공저 2명+인데 NLK/GPT가 원저자명 1명분만 반환. 700 필드도 이 단일
                    # orig_author_en을 첫 번째(주) 저자 것으로 보고 그 사람에게만 영문
                    # 도치를 쓴다(line ~2975 first_ai 처리) — 500도 같은 가정으로 첫
                    # 저자 한 명에 한정해서 채운다. "공저자 전체의 공통 원저자명"처럼
                    # 보이지 않도록 나머지 공저자는 500에 추가하지 않고 한글로만 둔다.
    ...
    if inferred_country:
        _llm_name_context_parts.append(f"저자국적: {inferred_country}")
    if lang_h:
        _llm_name_context_parts.append(f"원어코드: {lang_h}")
    ...
    if western_700_english:
        ...
        for a in persons_for_700:
            ind = marc700_second_indicator(
                ...
                inferred_country=inferred_country or "",
            )
            if a.get("role") in TRANS_ROLES:
                fields_700.append(f"700 {ind}_ {build_700(a, context=_llm_name_context, inferred_country=inferred_country or '')}")
            else:
                en = ko_to_en.get(_norm_author_lookup_key((a.get("name") or "").strip()))
                if en:
                    _en_name = en if (inferred_country or "") in _NO_INVERT_COUNTRIES else english_name_reverse(en)
                    fields_700.append(f"700 {ind}_ $a {_en_name}")
                    ...
                    _rom_name = orig_author_en.strip() if (inferred_country or "") in _NO_INVERT_COUNTRIES else english_name_reverse(orig_author_en.strip())
                    fields_700.append(f"700 {ind}_ $a {_rom_name}")
                    skip_first_person_700 = True
                else:
                    fields_700.append(f"700 {ind}_ {build_700(a, context=_llm_name_context, inferred_country=inferred_country or '')}")
    else:
        ...
        if (
            ...
            for en_chunk, ai_entry in zip(_orig_en_chunks, author_info):
                ...
                if fn and (ai_entry.get("is_east_asian") or orig_title_has_cjk_or_kana or ai_entry.get("kanji_name")):
                    ...
                    _ec_name = en_chunk.strip() if (inferred_country or "") in _NO_INVERT_COUNTRIES else english_name_reverse(en_chunk.strip())
                    fields_700.append(f"700 {ind}_ $a {_ec_name}")
        ...
        if first_ai and orig_author_en and not _handled_700_ko:
            ...
            if latin and (not first_ai.get("kanji_name") or translation_book):
                ...
                if looks_anthology_rom or looks_truncated_rom or mixed_lat_ko:
                    ...
                    _rom_inv = rom if (inferred_country or "") in _NO_INVERT_COUNTRIES else english_name_reverse(rom)
                    fields_700.append(f"700 1_ $a {_rom_inv}")
                    skip_first_person_700 = True
            elif (
                translation_book
                and not latin
        ...
        print(f"[700_loop] persons_for_700={[a['name'] for a in persons_for_700]} skip={skip_first_person_700} handled={_handled_700_ko} first_ko={first_ko_name!r}")
        for a in persons_for_700:
    ...
    fields_710 = [f"710 __ {build_710(a)}" for a in orgs]
        ...
        for idx, ai in enumerate(author_info):
            if ai.get("_500_script") == "cjk":
                ...
                for kn_part in kn_parts:
                    ...
                    if not kn_compact:
                        continue
                    ktype = kanji_type(kn_compact)
                    if ktype == "kanji_only":
                        reading = kanji_to_korean_reading(kn_compact)
                        if reading:
                            fields_900.append(f"900 10 $a {reading}")
                    elif ktype in ("kanji_kana", "kana_only"):
                        # 한자·가나 혼합 또는 가나만 있는 일본 이름(예: 上野そら, 朝倉かすみ)
                        # → 국중 관례대로 한자 구간은 한국 한자음, 가나 구간은 발음 그대로 음역해 이어붙임
                        gloss = _jp_mixed_name_reading_hangul(kn_compact)
                        if gloss:
                            fields_900.append(f"900 10 $a {gloss}")
                continue
    ...
    _sf_space = re.compile(r'\$([a-z]) ')

    def _fmt(s: str) -> str:
        return _sf_space.sub(r'$\1', s)

    marc: dict[str, object] = {
        "f245": _fmt(f"245 00 {field_245}"),
        "f246": _fmt(field_246) if field_246 else field_246,
        "f700": [_fmt(f) for f in fields_700],
        "f710": [_fmt(f) for f in fields_710],
    }
    if fields_500:
        f500 = fields_500 if len(fields_500) > 1 else fields_500[0]
        marc["f500"] = [_fmt(f) for f in f500] if isinstance(f500, list) else _fmt(f500)
    if fields_900:
        marc["f900"] = [_fmt(f) for f in fields_900]
    if field_940:
        marc["f940"] = _fmt(field_940)
    if tag_041:
        marc["f041"] = _fmt(tag_041)
    if tag_546:
        marc["f546"] = _fmt(f"546 __ $a{tag_546}")

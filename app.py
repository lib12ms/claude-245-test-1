...
[생성 필드]
  ...
  246 19  원서명 (번역서만)
  500 __  원저자명 주기 (번역서만)
  700 1_  개인명 부출(등록 성씨+이름 1~2음절로 성·이름 구분 가능)
  700 0_  개인명 부출(닉네임·필명·성씨 없음·이름 3음절 이상 등, 실명 예외 목록 제외)
  710 0_  기관명 부출기입
...
NLK_SEARCH_API_URL = "https://www.nl.go.kr/NL/search/openApi/search.do"
NLK_SEOJI_API_URL = "https://www.nl.go.kr/seoji/SearchApi.do"
NLK_SEOJI_API_URLS = (
    NLK_SEOJI_API_URL,
...
def _nlk_api_key() -> str:
    """국중 Open API 인증키. Render에 NLK_API_KEY 또는 (구) NL_API_KEY."""
    raw = os.environ.get("NLK_API_KEY") or os.environ.get("NL_API_KEY") or ""
...
def _nlk_redact_secrets(text: str) -> str:
    if not text:
        return text
    return re.sub(
        r"((?:cert_key|key)=)[^&\s\"']+",
...
def _nlk_sanitize_status(st: dict) -> dict:
    try:
        return json.loads(_nlk_redact_secrets(json.dumps(st, ensure_ascii=False, default=str)))
    except (TypeError, ValueError):
        return {"error": "status_serialize_failed"}


def _nlk_error_kind(exc: BaseException) -> str:
...
def _nlk_http_timeouts(budget_sec: float) -> tuple[float, float]:
    connect = float(os.environ.get("NLK_CONNECT_TIMEOUT", "18"))
    read = float(os.environ.get("NLK_READ_TIMEOUT", "22"))
    budget_sec = max(3, budget_sec)
    connect = min(connect, max(4, budget_sec * 0.6))
    read = min(read, max(6, budget_sec))
    return (connect, read)
...
def _nlk_get_json(
    ...
    ct, rt = _nlk_http_timeouts(budget_sec)
    last_kind = "unknown"
    for url in urls:
        for attempt in range(2):
            try:
                resp = requests.get(
                    ...
                    params=params,
                    timeout=(ct, rt),
                    headers=ALADIN_HEADERS,
                )
                resp.raise_for_status()
                return resp.json(), None
            except (requests.RequestException, ValueError) as e:
                last_kind = _nlk_error_kind(e)
                if attempt == 0 and last_kind in ("connect_timeout", "connection_error"):
                    time.sleep(0.4)
                    continue
                break
...
# 국중 표제/저자사항에 쓰이는 역할어(책임표시) 패턴
_NLK_RESP_ROLE_RE = re.compile(
    r"(지은이|옮긴이|저자|엮은이|그린이|역자|번역|편저|감수|해설|사진|원작|글쓴이|글)\s*:"
)
...
def fetch_orig_title_aladin_foreign(ko_title: str, author_names: list[str]) -> str | None:
    """
    알라딘 외국도서 검색으로 246용 원제(가나·한자) 후보.
    국중 서지 TITLE에 병행표제가 없거나 상세 조회가 실패할 때만 _isbn_lookup에서 사용.
    """
    base = strip_award_suffix_from_title(remove_series((ko_title or "").strip()))
    ...
    def _items(q: str, qtype: str) -> list[dict]:
        try:
            resp = requests.get(
                ALADIN_SEARCH_URL,
                params={
    ...
    best_score = -10**9
    for q in queries[:4]:
        for qtype in ("Keyword", "Title"):
            for it in _items(q, qtype):
                raw = re.sub(r"\s+", " ", (it.get("title") or "").strip())
                raw = re.sub(r"\s*[\(（][^)）]*[\)）]\s*$", "", raw)
                if not raw or not re.search(r"[\u3040-\u30ff\u4e00-\u9fff]", raw):
                    continue
                score = 110 - min(len(raw), 70)
                if base and base in re.sub(r"\s+", "", raw):
                    score += 60
                if score > best_score:
                    best_score = score
                    best = remove_year(raw)
    return best
...
# ─────────────────────────────────────────────
# 국중(국립중앙도서관) 책임표시 — 선택적 보강
# ─────────────────────────────────────────────
def _role_label_for_245(role: str) -> str:
    """245 책임표시에 쓸 역할어(국중 표제/저자사항 형식)."""
    r = (role or "").strip()
...
def _author_role_groups(authors: list[dict]) -> tuple[list[str], dict[str, list[str]]]:
    """책임표시용 역할별 이름 묶음 (등장 순서 유지)."""
    persons = [a for a in authors if not a["is_org"]]
    role_groups: dict[str, list[str]] = {}
    order: list[str] = []
    for a in persons:
        label = _role_label_for_245(a.get("role", ""))
        if label not in role_groups:
            role_groups[label] = []
            order.append(label)
        role_groups[label].append(a["name"].strip())
    return order, role_groups


def _build_responsibility_marc_suffix_from_authors(authors: list[dict]) -> str | None:
    """
    245 책임표시 MARC 접두.
    예: /$d 지은이: 아베 아키코 ;$옮긴이: 이소담
    """
    order, role_groups = _author_role_groups(authors)
    if not order:
        return None
    chunks: list[str] = []
    for i, label in enumerate(order):
        names = role_groups.get(label) or []
        if not names:
            continue
        body = f"{label}: {', '.join(names)}"
        if i == 0:
...
def _responsibility_plain_to_marc_suffix(text: str) -> str | None:
    """'지은이: A ; 옮긴이: B' → '/$d 지은이: A ;$옮긴이: B' (국중·NLK 평문)."""
    ...
    for i, part in enumerate(parts):
        if i == 0:
            chunks.append(f"/$d {part}")
            continue
        m = re.match(r"^([^:]+):\s*(.+)$", part)
        if m:
            chunks.append(f" ;${m.group(1).strip()}: {m.group(2).strip()}")
        else:
            chunks.append(f";$e {part}")
    return "".join(chunks) if chunks else None
...
_NLK_ORIG_TITLE_RE = re.compile(
    r"(?:원표제|원서명)\s*[:：]\s*(.+?)"
    r"(?=\s+(?:일본어|중국어|원작|원저|영문|한국어)|\s*[/;|]|$)"
)


def extract_orig_title_from_nlk_text(text: str) -> str | None:
    """국중 주기·서지 문자열에서 원표제/원서명 추출. 예: 원표제: カフネ"""
    if not text:
        return None
    candidates: list[str] = []
    for m in _NLK_ORIG_TITLE_RE.finditer(text):
        cand = re.sub(r"\s+", " ", m.group(1)).strip()
        for stop in (" 일본어", " 중국어", " 원작", " 영문", " 한국어", " 원저"):
            if stop in cand:
                cand = cand.split(stop, 1)[0].strip()
        if cand and len(cand) <= 80:
            candidates.append(cand)
    if not candidates:
        return None
    cjk = [c for c in candidates if re.search(r"[\u3040-\u30ff\u4e00-\u9fff\u3400-\u4dbf]", c)]
    pool = cjk if cjk else candidates
    return remove_year(min(pool, key=len))


def extract_orig_title_from_seoji_title(title_raw: str) -> str | None:
    """
    ISBN 서지 API TITLE에서 원표제 후보.
    예: '카프네 = カフネ : …' / '카프네 = Cafuné : …'
    """
    t = re.sub(r"\s+", " ", (title_raw or "").strip())
    if not t:
        return None
    hit = extract_orig_title_from_nlk_text(t)
    if hit:
        return hit
    if "=" not in t:
        return None
    right = t.split("=", 1)[1].strip()
    head = re.split(r"[:：]", right, 1)[0].strip()
    head = remove_year(head)
    if head and re.search(r"[\u3040-\u30ff\u4e00-\u9fff\u3400-\u4dbf]", head):
        return head
    par = extract_equals_parallel_title(t)
    if par:
        return par
    return None


def extract_equals_parallel_title(text: str) -> str | None:
    """'카프네 = Cafuné : …' 표제에서 = 뒤 병행표제(로마자 원제 후보)."""
    t = re.sub(r"\s+", " ", (text or "").strip())
    if "=" not in t:
        return None
    m = re.match(r"^[^=]+=\s*([^:：/;]+)", t)
    if not m:
        return None
    par = m.group(1).strip()
    if not par or re.search(r"[\uac00-\ud7a3]{4,}", par):
        return None
    if re.search(r"[\uac00-\ud7a3]", par) and re.search(r"[A-Za-z]", par):
        return None
    return remove_year(par)


def extract_responsibility_from_nlk_bib_text(text: str) -> str | None:
    """
    국중 서지 문자열에서 '/ 지은이: … ; 옮긴이: …' 책임표시만 추출.
    표제/저자사항 전체 또는 AUTHOR/TITLE 필드에 들어 있는 경우 모두 처리.
    """
    t = re.sub(r"\s+", " ", (text or "").strip())
    if not t:
        return None
    for sep in (" / ", "／", "/"):
        if sep in t:
            ...
            if _NLK_RESP_ROLE_RE.search(tail):
                return tail
    if _NLK_RESP_ROLE_RE.search(t):
        return t
    return None


def _nlk_collect_strings(obj, out: list[str], depth: int = 0) -> None:
    if depth > 8:
        return
    if isinstance(obj, str):
        ...
        for v in obj.values():
            _nlk_collect_strings(v, out, depth + 1)
    elif isinstance(obj, list):
        for v in obj[:40]:
            _nlk_collect_strings(v, out, depth + 1)


def _nlk_pick_orig_title(candidates: list[str]) -> str | None:
    if not candidates:
        return None
    cjk = [
        c for c in candidates
        if re.search(r"[\u3040-\u30ff\u4e00-\u9fff\u3400-\u4dbf]", c)
    ]
    pool = cjk if cjk else candidates
    return remove_year(min(pool, key=len))


def _nlk_scrape_orig_title(control_no: str, timeout: float = 8) -> str | None:
    """소장자료 상세 HTML의 원표제·원서명 라벨 (예: 원표제: カフネ)."""
    control_no = (control_no or "").strip()
    if not control_no:
        return None
    url = (
        "https://www.nl.go.kr/NL/contents/search.do"
        f"?pageNum=1&pageSize=10&srchTarget=total&kwd={quote(control_no)}"
    )
    try:
        resp = requests.get(url, headers=ALADIN_HEADERS, timeout=max(3, timeout))
        resp.raise_for_status()
        text = resp.text
    except requests.RequestException:
        return None
    candidates: list[str] = []
    for label in ("원표제", "원서명"):
        for m in re.finditer(
            rf"{label}\s*[:：]\s*(.+?)"
            r"(?=\s*내용|\s*일본어|\s*중국어|\s*영어|\s*ISBN|\s*원저자|\s*원표제|\s*원서명|$)",
            text,
        ):
            cand = remove_year(re.sub(r"\s+", " ", m.group(1)).strip())
            if cand and len(cand) <= 80:
                candidates.append(cand)
    return _nlk_pick_orig_title(candidates)


def _nlk_holdings_control_no_xml(isbn13: str, key: str, timeout: float = 7) -> str | None:
    """소장자료 Open API(XML) — control_no 추출."""
    ct, rt = _nlk_http_timeouts(timeout)
    try:
        resp = requests.get(
            NLK_SEARCH_API_URL,
            params={
    ...
    try:
        ...
        m = re.search(r"<control_no>\s*([^<]+)\s*</control_no>", text)
        return m.group(1).strip() if m else None
    for item in root.iter("item"):
        el = item.find("control_no")
...
def _nlk_seoji_rows(
    ...
    """ISBN 서지 API — docs 목록 (ISBN 조회 후 없으면 표제로 재시도)."""
    rows: list[dict] = []
    meta = status if status is not None else {}

    def _fetch(params: dict, via: str) -> None:
        nonlocal rows
        if rows and meta.get("result") == "SUCCESS":
            return
        data, err_kind = _nlk_get_json(NLK_SEOJI_API_URLS, params, timeout)
        if data is None:
            meta.setdefault("errors", []).append(f"{via}:{err_kind}")
            return
        result = (data.get("RESULT") or "").upper()
        if result == "ERROR":
            meta["result"] = "ERROR"
            meta["err_code"] = data.get("ERR_CODE") or data.get("errorCode")
            meta["err_msg"] = data.get("ERR_MESSAGE") or data.get("errorMsg")
            meta["via"] = via
            return
        meta["result"] = "SUCCESS"
        meta["via"] = via
        got = data.get("docs") or data.get("DOC") or []
        if isinstance(got, dict):
            got = [got]
        rows = [r for r in got if isinstance(r, dict)]
        meta["doc_count"] = len(rows)
        if rows:
            r0 = rows[0]
            t0 = (r0.get("TITLE") or r0.get("title") or "")[:200]
            meta["sample_title"] = t0
            cn = (r0.get("CONTROL_NO") or r0.get("control_no") or "").strip()
            if cn:
                meta["control_no"] = cn
    ...
    hint = re.split(r"\s*[-–:：]\s*", hint)[0].strip()
    if not rows and hint and len(hint) >= 2 and not meta.get("errors"):
        _fetch({
            "cert_key": key,
            "result_style": "json",
            "page_no": 1,
    ...
    for row in rows:
        ea = re.sub(
            r"[^\d]",
            "",
            str(row.get("EA_ISBN") or row.get("ea_isbn") or row.get("ISBN") or ""),
        )
        if ea and (ea == isbn_digits or ea.endswith(isbn_digits[-10:])):
            matched.append(row)
    return matched if matched else rows


def _nlk_orig_candidates_from_rows(rows: list[dict]) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    ...
    for row in rows:
        for val in row.values():
            ...
            if not s or len(s) > 500:
                continue
            _add(extract_orig_title_from_seoji_title(s))
            _add(extract_orig_title_from_nlk_text(s))
            _add(extract_equals_parallel_title(s))
    return out


def _nlk_holdings_json_orig_candidates(
    isbn13: str,
    key: str,
    timeout: float,
    status: dict | None = None,
) -> list[str]:
    """소장자료 Open API(JSON) — 응답 전체에서 원표제·병행표제 후보 수집."""
    out: list[str] = []
    meta = status if status is not None else {}
    params = {
        "key": key,
        "apiType": "json",
        "searchType": "isbn",
        "searchKeyword": isbn13,
        "pageNo": 1,
        "pageSize": 5,
    }
    data, err_kind = _nlk_get_json((NLK_SEARCH_API_URL,), params, timeout)
    if data is None:
        meta["error"] = err_kind or "request_failed"
        return out
    if isinstance(data, dict) and data.get("errorCode"):
        meta["error_code"] = data.get("errorCode")
        meta["error_msg"] = data.get("errorMsg")
        return out
    meta["ok"] = True
    blobs: list[str] = []
    _nlk_collect_strings(data, blobs)
    for s in blobs:
        hit = extract_orig_title_from_nlk_text(s)
        if hit:
            out.append(hit)
        ot = extract_orig_title_from_seoji_title(s)
        if ot:
            out.append(ot)
        par = extract_equals_parallel_title(s)
        if par:
            out.append(par)
    return out


def fetch_nlk_orig_title_by_isbn(
    isbn13: str,
    title_hint: str = "",
    status_out: dict | None = None,
) -> str | None:
    """
    알라딘 subInfo.originalTitle 이 없을 때만 246용 — 국립중앙도서관 Open API에서 원표제 조회.
    환경변수 NLK_API_KEY 또는 NL_API_KEY(구 이름) 필요.
    Render 공개 URL은 약 30초 안에 응답해야 하므로 NLK_FETCH_BUDGET_SEC(기본 14초) 내에 끝냄.
    """
    st: dict = status_out if status_out is not None else {}
    key = _nlk_api_key()
    if not key:
        st["skipped"] = "no_key"
        return None
    isbn13 = re.sub(r"[^\d]", "", isbn13 or "")
    if len(isbn13) not in (10, 13):
        st["skipped"] = "bad_isbn"
        return None

    budget = float(os.environ.get("NLK_FETCH_BUDGET_SEC", "20"))
    ...
    rows: list[dict] = []
    if _left() > 0:
        rows = _nlk_seoji_rows(
            isbn13, key, min(12, max(4, _left())), title_hint, seoji_st
        )
        st["seoji"] = seoji_st
        orig_candidates.extend(_nlk_orig_candidates_from_rows(rows))

    seoji_ok = seoji_st.get("result") == "SUCCESS" and bool(rows)

    # 서지 API에 책이 있으면 CONTROL_NO 상세(원표제)에 남은 시간을 몰아줌.
    # 소장 JSON/XML은 Render에서 자주 타임아웃나며, 서지 성공 뒤에는 불필요한 경우가 많음.
    if seoji_ok and _left() > 0:
        for row in rows[:3]:
            cn = (row.get("CONTROL_NO") or row.get("control_no") or "").strip()
            if not cn:
                continue
            scraped = _nlk_scrape_orig_title(cn, timeout=max(4, _left()))
            if scraped:
                orig_candidates.append(scraped)
                st["scrape_via"] = f"seoji_control_no:{cn}"
                break

    picked = _nlk_pick_orig_title(orig_candidates)
    if picked:
        st["picked"] = picked
        return picked

    if not seoji_ok and _left() > 0:
        orig_candidates.extend(
            _nlk_holdings_json_orig_candidates(
                isbn13, key, min(8, max(3, _left())), hold_st
            )
        )
        st["holdings_json"] = hold_st
        picked = _nlk_pick_orig_title(orig_candidates)
        if picked:
            st["picked"] = picked
            return picked
    ...
    if not seoji_ok:
        control_no = _nlk_holdings_control_no_xml(
            isbn13, key, timeout=min(7, max(3, _left()))
        )
        st["holdings_control_no"] = control_no
        if control_no and _left() > 0:
            scraped = _nlk_scrape_orig_title(
                control_no, timeout=min(10, max(4, _left()))
            )
            if scraped:
                st["picked"] = scraped
                st["scrape_via"] = f"holdings_xml:{control_no}"
                return scraped
...
def _isbn_lookup():
    ...
    nlk_configured = bool(_nlk_api_key())
    nlk_status: dict = {}
    if has_translator:
        # 국중(246)을 먼저 — Render→nl.go.kr 연결이 느릴 때 알라딘 크롤과 겹치면 타임아웃
        nlk_title = None
        if nlk_configured:
            nlk_title = fetch_nlk_orig_title_by_isbn(isbn13, title, nlk_status)
        orig_info = collect_orig_info(item, item_id, isbn13, title, authors)
        orig_title     = orig_info["orig_title"] or nlk_title
        if not orig_title:
            non_trans = [
                a["name"] for a in authors
                if not a["is_org"] and a["role"] not in TRANS_ROLES
            ]
            af_title = fetch_orig_title_aladin_foreign(title, non_trans)
            if af_title:
                orig_title = af_title
                nlk_status["fallback_246"] = "aladin_foreign"
        if nlk_configured:
            nlk_status["configured"] = True
            nlk_status["used_for_246"] = bool(orig_title)
            nlk_status["from_nlk_api"] = bool(nlk_title)
    ...
    # 500: 원저자명(한자·영문). 한자는 저자별(프로필·키워드 검색으로 최대한 채움), 없으면 일본 맥락에서만 한글 표기.
    ...
    payload = {
        ...
        "marc":       marc,
        "nlk_configured": nlk_configured if has_translator else False,
        "nlk_status": (
            _nlk_sanitize_status(nlk_status)
            if has_translator and nlk_configured and nlk_status
            else None
        ),
    }
    try:

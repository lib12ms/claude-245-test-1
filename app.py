...
def _nlk_seoji_rows(
    ...
    def _fetch(params: dict, via: str) -> None:
        ...
        if rows:
            t0 = (rows[0].get("TITLE") or rows[0].get("title") or "")[:120]
...
def _nlk_orig_candidates_from_rows(rows: list[dict]) -> list[str]:
    ...
    for row in rows:
        ...
        if title_raw:
            ot = extract_orig_title_from_seoji_title(title_raw)
            if ot:
                out.append(ot)
            hit = extract_orig_title_from_nlk_text(title_raw)
            if hit:
                out.append(hit)
            par = extract_equals_parallel_title(title_raw)
            if par:
                out.append(par)
        for fld in ("AUTHOR", "author", "TITLE_INFO", "title_info"):
            val = row.get(fld)
            if val:
                s = str(val)
...
def fetch_nlk_orig_title_by_isbn(
    ...
    if _left() > 0:
        rows = _nlk_seoji_rows(
            isbn13, key, min(8, max(3, _left())), title_hint, seoji_st
        )
        st["seoji"] = seoji_st
    ...
    if control_no and _left() > 0:
        scraped = _nlk_scrape_orig_title(control_no, timeout=min(8, max(3, _left())))
        if scraped:
            st["picked"] = scraped
...
def _isbn_lookup():
    ...
    if has_translator:
        ...
        if nlk_configured:
            ...
            nlk_status["used_for_246"] = bool(nlk_title)

...
def _nlk_search_one(kwd: str, key: str) -> str | None:
    ...
    try:
        ...
        print(f"[NLK] search request failed kwd={kwd!r}: {e}", flush=True)
        return None

    if "INVALID KEY" in resp.text or "<error>" in resp.text[:500].lower():
        print(f"[NLK] invalid key or error for kwd={kwd!r}", flush=True)
        return None

    print(f"[NLK] search kwd={kwd!r} resp[:300]={resp.text[:300]!r}", flush=True)
    try:

...
def _parse_marc_orig_info(marc_text: str) -> dict:
    ...
    has246 = "246" in plain
    has500 = "원저자명" in plain
    print(f"[NLK DEBUG] has_246={has246} has_원저자명={has500} plain[:400]={plain[:400]!r}", flush=True)
    ...
    # 7) 폴백: 100/700 1_ 필드의 로마자 이름 추출
    #    500 원저자명이 없어도 100/700에 "Saint-Exupéry, Antoine de" 형태로 존재
    #    역도치(Last, First) → 정도치(First Last) 변환 후 반환
    if not result["orig_author"]:
        for m in re.finditer(
            rf"[17]00\s+1[^\n]{{0,8}}{_SF}\s*a\s*([A-Za-zÀ-ɏ][^▼$\n\r]{{3,80}})",
            plain, re.I,
        ):
            raw = m.group(1).strip().rstrip(".,; ")
            if not raw or re.search(r"[가-힣぀-ヿ一-鿿]", raw):
                continue
            if "," in raw:
...
def fetch_nlk_orig_info(isbn13: str, title: str = "") -> dict:
    ...
    print(f"[NLK] isbn={isbn} title={title!r:.30} record_id={record_id!r}", flush=True)
    if not record_id:
        return empty

    marc_text = _fetch_nlk_marc_text(record_id)
    print(f"[NLK] marc_text_len={len(marc_text) if marc_text else 0}", flush=True)
    if not marc_text:
        return empty

...
def parse_responsibility_entries(resp_suffix: str) -> list[dict]:
    ...
    if first:
        current_role, names = _parse_role_name_block(first, current_role)
        for name in names:
    ...
    while i < len(tokens):
        ...
        if not content:
            continue
        if ";$e" in delim:
            current_role, names = _parse_role_name_block(content, current_role)
            for name in names:
                entries.append({"role": current_role, "name": name})
        else:
            for name in re.split(r"\s*,\s*", content):
                if name.strip():
                    entries.append({"role": current_role, "name": name.strip()})
    return entries

...
if "data" in st.session_state:
    ...
    f041 = marc.get("f041", "")
    if f041:
        st.markdown('<div class="marc-label">041 — 언어코드</div>', unsafe_allow_html=True)
        st.code(f041, language=None)

    f546 = marc.get("f546", "")
    if f546:
        st.markdown('<div class="marc-label">546 — 언어주기</div>', unsafe_allow_html=True)
        st.code(f546, language=None)
    ...
    if f041:
        all_fields.append(f041)
    if f546:
        all_fields.append(f546)
